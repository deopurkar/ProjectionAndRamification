...
\begin{remark}
  A consequence of the proof of \autoref{prop:quarticscroll} is the following.
  Let \(N\) be a net of conics in \(\mathbf{P}S\) containing the distinguished conic \(q\).
  Let \(E \subset \mathbf{P}S^{*}\) be the Hermite cubic of \(N\).
  We have a distinguished non-trivial 2-torsion element \(\epsilon_0\) in \(\Pic(E)\) corresponding to the natural involution of \(E\).
  The two points in the pre-image of \(N\) under \(\rho\) correspond to the other two non-trivial 2-torsion elements in \(\Pic(E)\).
\end{remark}
\end{document}
