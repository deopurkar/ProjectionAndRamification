...
\begin{proof}
  ...
  Therefore, $\dim P \leq \dim |L|$, and $k \leq n-r+1$.

  To show that $k = n-r$, it suffices to show that $\sigma \from P \to |L|$ is not surjective.
  We do so using Bertini's theorem.
  Let $B \subset X$ denote the base scheme of the linear series $|L|$, and let $\pi \from \widetilde X = \Bl_{B}X \to X$ be the blow-up, with exceptional divisor $E$.
  Then the linear series $|L|$ viewed as a sub-series of $|\pi^*L - E|$ is basepoint free.
  We define a modified conormal variety $\widetilde P$ by setting
  \[\widetilde{P} = \overline{\left\{(x,D) \mid x \in \widetilde{X} \text{ general}, \,\, x \in \sing D, D \in|L|\right\}} \subset \widetilde{X} \times |L|.\]
  Note that, if the open set $U \subset X$ in the definition of $P$ is chosen to be disjoint from $B$, then $\widetilde P$ is the closure of $\P(K|_U)$ in $\widetilde X \times |L|$.
  In particular, $\widetilde P$ is irreducible of the same dimension as $P$.
  We also have a birational morphism $\widetilde P \to P$, obtained by the restricting the natural map $\widetilde X \times |L| \to X \times |L|$.
  Let $\widetilde P^{\rm sing} \subset \widetilde P$ be the pre-image of the singular locus $\widetilde X^{\rm sing}$ of $\widetilde X$.
  Then $\widetilde P^{\rm sing} \subset \widetilde P$ is a proper closed subset, and hence
  \[ \dim \widetilde P^{\rm sing} < \dim \widetilde P.\]
  Since we know that $\dim \widetilde P = \dim P \leq \dim |L|$, we conclude that $\widetilde P^{\rm sing} \to |L|$ is not surjective.
  Let $D \in |L|$ be a general element, in particular, not in the image of $\widetilde P^{\rm sing}$.
  By Bertini's theorem, $D$ is non-singular.
  Therefore, $D$ is not in the image of $\widetilde P$, and hence $\widetilde P \to |L|$ is not surjective.
  Since the projection $\widetilde P \to |L|$ factors as $\widetilde P \to P \to |L|$, and the first map $\widetilde P \to P$ is a birational morphism (in particular, surjective), we conclude that $P \to |L|$ is not surjective.
