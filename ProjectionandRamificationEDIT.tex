...
We prove \autoref{theorem:rationalnormalscrolls} by degeneration, using the theory of limit linear series of higher rank developed by Osserman \cite{oss:14}. In the course of proving the theorem, we also demonstrate dominance in the case of $E = \O(2)^{r}$.
...
As a corollary, we obtain the claim in \autoref{Thm:Counterexamples}:


\begin{corollary}
    There exist non-degenerate, smooth projective varieties $X \subset \P^{n}$ of every dimension $r \geq 4$ for which the projection-ramification map is not generically finite.
    \label{NonDominance}
\end{corollary}

\begin{proof}
    Take $X = \P E$ appearing in \autoref{prop:specialE} embedded by the complete linear series $\O_{X}(1)$.  The projection-ramification map $\rho_{X}: \Gr(r+1, H^0(\P^1, E)) \dashrightarrow \P H^0(\P^1, E \otimes \det E \otimes K_{\P^1})$ is evidently $\Aut (\P E/\P^{1})$-equivariant.  The corollary immediately follows from this equivariance, \autoref{prop:trivialStabilizer}, and \autoref{prop:specialE}.
\end{proof}
