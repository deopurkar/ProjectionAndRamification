...
\begin{proof}
  ...
  It follows that $\rho$ is a local immersion at $\lambda$, and hence dominant.

  Our proof is by direct calculation.
  We work over $\A^1 = \spec \k[x] \subset \P^1$ and identify $\O(n)$ with $\O(n \cdot \infty)$.
  Then the global sections of $\O(n)$ are identified with polynomials in $x$ of degree at most $n$.
  Denote the generator of the $i$th summand of $E$ on $\A^1$ by $X_i$.
  Consider the point of $\Gr(r+1, H^0(\P^1, E))$ represented by the vector space $V \subset H^0(\P^1, E)$ spanned by $(r+1)$ sections $v_1, \dots, v_{r+1}$ defined as follows.
  ...
  To do the tangent space computation, we choose vectors $w_i \in H^0(\P^1, E)$, and change $v_i$ to $v_i + \epsilon w_i$, where $\epsilon^2 = 0$.
  We have
  \[ R_\epsilon = \det M(v_1 + \epsilon w_1, \dots, v_{r+1} + \epsilon w_{r+1}) = R + \epsilon S(w_1, \dots, w_{r+1}),\]
  where
  \[S \from H^0(\P^1, E)^{r+1} \to H^0(\P^1, E \otimes \O(2r))\]
  ...
  By our calculations above, $S$ maps $W_i$ to the subspace of $H^0(\P^1, E \otimes \O(2r))$ spanned by $X_i \otimes H^0(\P^1, \O(2r))$.
  ...
  \]
  We claim that, there is no non-trivial linear relation among the polynomials $Q_{i,j}$ for $j \in \{1, \dots, r+1\} \setminus \{i\}$.
  ...
  If $l_j \neq 0$ for some $j$ with $1 \leq j \leq r$, then we have a pole on the left side at $x = a_j$, but not on the right side.
