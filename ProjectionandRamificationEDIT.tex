...
\begin{abstract}
  ...
  We give a geometric explanation of the fibers in terms of torsion points of naturally ocurring elliptic curves in the case of the Veronese surface and the quartic rational surface scroll.
\end{abstract}
...
Some more examples, calculated using randomized trials over finite fields in \texttt{Macaulay2} \cite{gra.sti:} and \texttt{Magma} \cite{bos.can.pla:97}, are tabulated in \autoref{tab:computation}.
 There seems to be some enchanting combinatorics behind $\phi$. As a sample, we point out that the sequence of numbers $1,2,6,22, 92, 422$ appearing down the diagonal in \autoref{tab:computation} are the number of Baxter permutations on $n=1,2, \dots, 6$ letters.  We plan on conducting a more complete enumerative investigation of $\phi$ in a future paper. 
\begin{table}
  \centering
