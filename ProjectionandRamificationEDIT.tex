...
A fundamental object associated to $f$ is the set $R(f) \subset X$ consisting of critical points of $f$---the points $x \in X$ at which $df \from T_xX \to T_{f(x)}Y$ has less than maximal rank.
The broad goal of this paper is to investigate the extent to which $f$ can be recovered from $R(f)$.
Particularly, we focus on the case when $X$ is a smooth projection variety and $Y$ is a projective space of the same dimension as $X$, and treat the following two questions: Does every non-trivial perturbation of $f$ induce a non-trivial perturbation of $R(f)$? If this is the case, then how many other $g \from X \to Y$ have $R(g) = R(f)$?
...
\]
is defined by $\rho \from L \to R(L)$.
...
We call $X \subset \P^n$ satisfying \eqref{item:incomp} \emph{incompressible}---it cannot be projected down (``compressed'') to a smaller dimensional subvariety.
The maximal variation result in\cite{fle.man:98} is equivalent to \eqref{item:incomp} of \autoref{thm:main}.
Our proof is different, and much easier.
...
In contrast, that $X$ may be compressible.
