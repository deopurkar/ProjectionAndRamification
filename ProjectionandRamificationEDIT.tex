...
We prove \autoref{theorem:rationalnormalscrolls} by degeneration, using the theory of limit linear series of higher rank developed by Teixidor i Bigas \cite{tei:} and Osserman \cite{oss:14}.
...
We need limit linear series only in the simplest singular curve, namely a (projective, connected) nodal curve $C$ which is the nodal union of two smooth (projective, connected) curves $C_1$ and $C_2$, but we need them for vector bundles of rank higher than $1$.
Let $B$ be a DVR with special point $0$, general point $\eta$.
...
In particular, for sufficiently positive $n$, say for $n \geq n_1$, we have $H^0(C_1, \mathcal E_n|_{C_1}) = 0$, and similarly, for sufficiently negative $n$, say $n \leq n_2$, we have $H^0(C_2, \mathcal E_n|_{C_2}) = 0$.
Assume, without loss of generality, that $n_1 \geq n_2$.
Set
\[ d_1 = w_1 - n_1 r, \text{ and } d_2 = w_2 + n_2 r, \text{ and } b = n_1 - n_2.\]
Observe that
\[ d_1 + d_2 - rb = d.\]
...
\begin{definition}[Limit linear series]
  \label{def:lls}
  Let $S$ be a $B$-scheme.
  A \emph{$k$-dimensional limit linear series} on $\mathcal E$ consists of sub-bundles $V_n \to \pi_* \mathcal E_n$ of rank $k$ for every $n \in \Z$ satisfying the following compatibility condition.
  For every $m, n \in \Z$, the map
  \begin{equation}\label{lls:compatibility}
    \pi_* \theta_n \from \pi_* \mathcal E_m \to \pi_* \mathcal E_{m+n} \text{ maps } V_n \to V_{m+n}.
  \end{equation}
\end{definition}
...
\end{theorem}
The last statement implies the following.
If $\lambda$ is a simple limit linear series such that the fiber of $\beta$ through $\lambda$ has dimension at most $k(d-k-r(g-1))$ at $\lambda$, then $\beta$ is open at $\lambda$ of relative dimension exactly $k(d-k-r(g-1))$.
...
Let $V_0 \subset E|_p \cong E(-ap)|_p$ be the image of $V(-ap)$, and $V_1 \subset E|_p \cong E(-(a+1)p)|_p$ the image of $V(-(a+1)p)$.
Then $\dim V_0 = i$ and $\dim V_1 = r+1-i$.
Assume that $V_0$ and $V_1$ satisfy the following genericity condition
\begin{equation}\label{eq:genericity}
  \dim (V_0 \cap V_1) = 1.
\end{equation}
Recall that the ramification divisor $R(V)$ is given by a section $r_V$ of $E \otimes \det E \otimes K_C$.
\begin{proposition}\label{prop:agreement}
  In the setup above, $r_V$ vanishes to order $(r+1)a + (r-i)$ at $p$.
  Write $\widetilde r = r_V/t^{(r+1)a + r-i}$, where $t$ is a uniformizer at $p$.
  Then, the one-dimensional subspace of $E|_p$ spanned by $\widetilde r|_p$ is $V_0 \cap V_1$.
\end{proposition}
\begin{proof}
  ...
  \[ V = \{t^as_1, \dots, t^as_i, t^{a+1}s_i, t^{a+1}s_{i+1}, \dots, t^{a+1}s_{r}\}.\]
  By \eqref{eqn:Rmatrix}, in these coordinates, $r_V$ is given by
  \begin{align*}
    r_V &= \det
    \begin{pmatrix}
      t^a & & & & & & at^{a-1}s_1\\
  ...
  \end{align*}
  Since $s_i$ spans $V_0 \cap V_1$, the proof is complete.
\end{proof}
...
\begin{equation}\label{eq:Rtilde}
  \rho \from {\mathcal G}^{r+1}_{r,d,d_*,w_*}(X/B)^{\rm red} \dashrightarrow {\mathcal G}^{1}_{r',d',d'_*,w'_*}(X/B)
\end{equation}
...
\begin{enumerate}
\item the closure of the locus of linear series $(\mathcal E, V)$ on the generic fiber $X_\eta$ such that $V \otimes \O_{X_\eta} \to \mathcal E$ has generic rank less than $r$,
\item the locus of limit linear series $(\mathcal E, V_n)$ on the central fiber such that the associated EHT limit linear series $\mu = (\mathcal E, W_1, W_2)$ is not refined, or does not have vanishing sequences as in \eqref{eqn:specialvs}, or does not satisfy the genericity condition $\dim (V_0 \cap V_1) = 1$ as in \eqref{eq:genericity}.
\end{enumerate}
...
\begin{proof}
  ...
  Note that the spaces $V_0$ and $V_1$ are exchanged when we switch from $C_1$ to $C_2$, and so their intersection $V_0 \cap V_1$ remains the same.
  By \autoref{prop:agreement}, after dividing by the appropriate power of the uniformizer, the sections $r_{n_1}$ and $r_{n_2}$ at $p$ are proportional; they both span $V_0 \cap V_1$.
...
\begin{equation}\label{eqn:llsR}
  \rho \from \mathcal U^{\rm red} \to \mathcal G^1_{r',d',d'_*,w'_*(X/B)}
\end{equation}
defined by
...
over $p$.
Given $\lambda \in U$, let $\widetilde r(\lambda) \in \P H^0(E \otimes \det E \otimes K_{\P^1} \otimes \O(-(r-1)p)^*$ be the reduced ramification section; see \autoref{prop:agreement}.
...
\begin{equation}\label{eqn:rrd}
  \widetilde \rho \from U \to \P H^0(E \otimes \det E \otimes K_{\P^1} \otimes \O(-(r-1)p))^*
\end{equation}
...
\begin{proposition}\label{prop:domred}
  ...
  \[ \rho \from \Gr(r+1, H^0(E'_{\rm gen})) \dashrightarrow \P H^0(E'_{\rm gen} \otimes \det E'_{\rm gen} \otimes K_{\P^1})^*\]
...
\begin{proof}
  ...
  Since $\rho$ is dominant for $E'$, there exists a sequence of subspaces $V_n' \in \Gr(r+1, H^0(E'))$ such that $\rho(V_n')$  limit to $D'$.
  Let $V_n \subset \Gr(r+1, H^0(E))$ be the image of $V'_n$.
  Then $\widetilde \rho(V_n)$ limit to $D$.
