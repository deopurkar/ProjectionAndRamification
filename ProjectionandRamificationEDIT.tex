...
In \autoref{sec:generic}, we prove \autoref{thm:rationalnormalscrolls} as \autoref{thm:actualrationalnormalscrolls}, after treating some low degree examples by hand in \autoref{sec:lowdegree}, introducing (linked) limit linear series for vector bundles of higher rank in \autoref{sec:lls}, and defining the projection-ramification map for linked linear series in \autoref{sec:prnongeneric} and \autoref{sec:prlls}.
...
\begin{proof}
  ...
  It is also easy to check that $V = (V_n \mid n \in \Z)$ a linked linear series, the only one that satisfies $V_{n_i} = \mathcal W_i$.
  The proof is now complete.
\end{proof}
