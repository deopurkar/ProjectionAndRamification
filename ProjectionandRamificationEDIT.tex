...
Then $\rho(N)$ corresponds to the cubic curve traced out by the nodes of the singular members of $N$, called the \emph{Steinerian} of $N$.
\begin{proposition}
  Let $C \subset \P^2$ be a general cubic.
  The fiber of $\rho$ over $C$ is in natural bijection with the set of non-trivial $2$-torsion line bundles on $C$.
  In particular, we have $\deg \rho = 3$.
\end{proposition}
The rest of \autoref{sub:veronese} is devoted to the proof of this assertion.

For the proof, we recall some classical projective geometry of nets of conics from \cite{dolgachev}.
...
The net $N$ gives rise to two cubic curves, the Steinerian, and the Cayleyan, both of which play a crucial role.
First, let $D \subset \P N^*$ be the locus of singular conics in $N$.
It is easy to see that $D$ is itself a plane cubic, called the \emph{discriminant cubic}.
In fact, $D$ is naturally isomorphic to the Steinerian $C \subset \P V$ introduced above by the map
\[ D \ni x \mapsto \text{Singular point of $Q_x$} \in \P V.\]
Note that $Q_x$, being a singular conic in a general net, is the union of two lines.
Such lines are called \emph{Reye lines}.
More precisely, a \emph{Reye line} is a line that is a component of $Q_x$ for some $x \in \P N^*$ (equivalently, $x \in D$).
Let $C' \subset \P V^*$ be the locus of Reye lines.
A simple calculation shows that $C'$ is \emph{also} a plane cubic; it is called the \emph{Cayleyan} of $N$.
The Cayleyan is not isomorphic, in general, to the Steinerian or the discriminant, but is closely related.
Evidently, we have a degree 2 map
\[ C' \to D\]
given by
\[ L \mapsto x,\]
if $L$ is a component of $Q_x$.
Said differently, we have a (fixed-point free) involution on the Cayleyan $C'$ given by
\[ L \mapsto L',\]
where $L'$ is the other component of the singular conic containing $L$.
The discriminant $D$ (and the Steinerian $C$) is the quotient of $C'$ by this involution.
The \'etale double cover $C' \to C$ defined above gives an non-trivial $\eta \in \Jac(C)[2]$.

Let $U \subset \P H^0(\P^2, \O(3))$ be the locus of smooth cubic curves, and let $J \to U$ be the Jacobian of the universal cubic over $U$.
Let $J[2] \subset J$ be the locus of 2-torsion elements, and $J[2]^* \subset J$ the locus of non-trivial 2-torsion elements.
The discussion above shows that the projection-ramification map
\[ \rho \dashrightarrow \Gr(3, H^0(\P^2, \O(2))) \to \P H^0(\P^2, \O(3))\]
factors as
\begin{alignat*}{2}
  \Gr(3, H^0(\P^2, \O(2))) &\dashrightarrow J[2]^*& &\dashrightarrow \P H^0(\P^2, \O(3))\\
  N &\mapsto (C, \eta)& &\mapsto C.
\end{alignat*}




Let $D \subset \P N^*$ be the locus of singular conics in this set, called the \emph{discriminant curve}.
It is easy to check that $D \subset \P N^*$ is a plane cubic.
In fact, we have an embedding $D \subset \P^2$

An element of $D$ corresponds to a singular element of $N$, which is the union of two lines in $\P^2$.
A line in $\P^2$ which is a component of a singular element of $N$ is called a \emph{Reye line}.
A simple calculation shows that the locus of 
