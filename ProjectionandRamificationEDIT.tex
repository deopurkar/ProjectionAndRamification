...
For a fixed $L$, the set of all projection $(L, V)$ are parametrized by the Grassmannian 
\begin{align*}
  \Gr(n+1, H^{0}(X,L)).
\end{align*}
...
We begin by proning part \eqref{item:incomp}, which is substantially easier.
...
\begin{proof}
  ...
  By \autoref{prop:projram}, this implies that $(L, V)$ is properly ramified.
  Hence, the sub-grassmannian $\Gr(r+1, W) \subset \Gr(r+1, H^0(X,L))$ lies entirely in the open subset of properly ramified map, where $\rho_{X,L}$ is regular.
...
The proof of part \eqref{item:dual} of \autoref{theorem:Main}, we proceed inductively by showing that a general $(n-r-1)$-dimensional linear subspace which is incident to $X$ is an isolated point in its fiber under $\rho_X$.
