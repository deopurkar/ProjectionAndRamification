...
The right definition requires more sophisticated tools, specifically, the spaces of (linked) limit linear series for vector bundles of higher rank developed by Teixidor i Bigas \cite{tei-i-big:91} and Osserman \cite{oss:14}.
...
The points of the fiber of $\rho$ over $(R, \eta)$ correspond naturally to the two elements of $\Pic E[2] \setminus \pi^* \Pic R [2]$, where $E \to R$ is the \'etale double cover defined by $\eta$.
...
For an ideal sheaf $I \subset \O_X$ we denote by $W \otimes I$ the subspace of $W$ consisting of the sections that vanish modulo $I$. More precisely, if $K$ is the kernel of the evaluation map
...
\subsection{Eccentric threefold scrolls} \label{sec:eccentric_threefolds}
...
\section{The Projection-Ramification enumerative problem} \label{sec:enumerativeproblems}
...
\subsection{Quadric hypersurfaces} \label{sec:aquadricsurface}
...
\subsection{The Veronese surface} \label{sec:veronese}
...
\begin{proof}
  ...
  \end{align*}
  It is easy to check that this map is a birational isomorphism---a general triple of points in $\P S$ is the zero locus of $2 \times 2$ minors of a matrix of linear forms, which is uniquely determined up to row and column transformations.
...
\begin{proof}
  ...
  Indeed, in the pencil of conics spanned by $A_1 \cup B_1$ and $A_2 \cup B_2$, the third singular conic is $\langle p, t\rangle \cup \langle p', t'\rangle$, where $p' = A_1 \cap B_2$ and $t' = A_2 \cap B_1$.
...
{\sc W.~Bosma, J.~Cannon, and C.~Playoust}, {\em The {M}agma algebra system.
  {I}. {T}he user language}, J. Symbolic Comput., 24 (1997), pp.~235--265.
\newblock Computational algebra and number theory (London, 1993).

\bibitem{dol.how:15}
{\sc I.~Dolgachev and B.~Howard}, {\em Configuration spaces of complex and real
  spheres}, in Recent advances in algebraic geometry, vol.~417 of London Math.
  Soc. Lecture Note Ser., Cambridge Univ. Press, Cambridge, 2015, pp.~156--179.
...
\bibitem{eke.lan.sha.ea:99}
{\sc T.~Ekedahl, S.~Lando, M.~Shapiro, and A.~Vainshtein}, {\em On {H}urwitz
  numbers and {H}odge integrals}, C. R. Acad. Sci. Paris S\'{e}r. I Math., 328
  (1999), pp.~1175--1180.

\bibitem{eke.lan.sha.ea:01}
\leavevmode\vrule height 2pt depth -1.6pt width 23pt, {\em Hurwitz numbers and
  intersections on moduli spaces of curves}, Invent. Math., 146 (2001),
  pp.~297--327.
...
\bibitem{gra.sti:}
{\sc D.~R. Grayson and M.~E. Stillman}, {\em Macaulay2, a software system for
  research in algebraic geometry}.
\newblock Available at \url{http://www.math.uiuc.edu/Macaulay2/}.

\bibitem{kul:08}
{\sc V.~S. Kulikov}, {\em On {C}hisini's conjecture. {II}}, Izv. Ross. Akad.
  Nauk Ser. Mat., 72 (2008), pp.~63--76.

\bibitem{muk:12}
{\sc S.~Mukai}, {\em Igusa quartic and {S}teiner surfaces}, in Compact moduli
  spaces and vector bundles, vol.~564 of Contemp. Math., Amer. Math. Soc.,
  Providence, RI, 2012, pp.~205--210.
...
\bibitem{the:17}
{\sc {The Sage Developers}}, {\em {S}ageMath, the {S}age {M}athematics
  {S}oftware {S}ystem ({V}ersion 7.5.1)}, 2017.
