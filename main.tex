...
Surprisingly, we know very little about $\rho$, despite its evident importance in projective geometry.
...
It is easy to check that $\rho$ is an isomorphism for quadric hypersurfaces.
For the Veronese surface in $\P^5$, the map $\rho$ can be identified with the map that sends a net of conics in $\P^2$ to its Jacobian cubic; this map has degree 3 (see \cite[Exercise~3.2 and 3.12]{dol:12}).
...
As a result, it is immediate that its degree is the degree of the Grassmannian in its Pl\"ucker embedding, which is the Catalan number $\frac{(2n-2)!}{n!(n-1)!}$.
For scrolls of dimension 2 and higher, the degree of $\rho$ remains unknown (but see \autoref{prop:segre} for some cases).
...
a sequence which appears to be the beginning of \cite[A001181]{oei:} and suggests an as-yet-mysterious combinatorial structure in the degrees of $\rho$.

A second natural set of questions is about the behavior of $\rho$ in characteristic $p$ and over the real numbers.
...
We thank Fyodor Zak for generously sharing comments, ideas, and encouragement over several months.
...
In this section, we define a projection-ramification map for linear series on a proper, normal, variety $X$.
...
Working with abstract linear series offers more flexibility that is helpful in inductive proofs.
...
\end{definition}
If $L$ is clear from context, we omit it and denote the ramification divisor by $R(V)$.
\begin{remark}\label{rem:Jacobian}
  ...
  Since $d \phi$ is given locally be the Jacobian matrix, the ramification divisor $R(L, V)$ is also called the \emph{Jacobian} of the linear series $(L, V)$ (see, for example, \cite[1.1.7]{dol:12}).
\end{remark}
...
By pushing forwad to $X$ and taking global sections, we get
...
Note that $v(s) = v(\lambda s)$ for a non-zero scalar $\lambda$, so we may talk unambiguously about $v(s)$ for $s \in |W|$.
...
Therefore, $R \subset X$ is a sub-scroll---the fibers of $R \to C$ are hyperplanes in the corresponding fiber of $X \to C$.
An explicit description of these hyperplanes, obtained either using the original definition or using the differential construction, is as follows.
...
We now write an equation of $R(V) \subset X$ over an open subset of $C$ containing $c$ explicitly in coordinates.
...
\todo{Summarize surface and threefold scrolls as a remark.}
...
\begin{remark}
The notion of a sub-bundle of a push-forward is subtle; it is treated in depth in \cite[Definition~B.2.1]{oss:14}.
...
\autoref{prop:llseht} allows us to combine the economy a EHT limit linear series with the functorial definition of a linked linear series.
...
%%% Local Variables:
%%% mode: latex
%%% TeX-master: "ProjectionandRamificationEDIT"
%%% End:
